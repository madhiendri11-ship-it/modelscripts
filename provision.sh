...
#    AND comfyui_controlnet_aux (DWPose pose-extraction preprocessor)
...
#                 + Z-Image-Turbo-Fun-Controlnet-Union (pose-only control)
...
if [ "$ZIMAGE" = "true" ]; then
    ...
    # Pose-only reference control -- lets a reference photo drive JOINT
    # POSITIONS only (via DWPose skeleton extraction), with zero body
    # proportion/skin/identity data leaking through. Confirmed node schema
    # from Comfy-Org's official workflow template + alibaba-pai's own
    # model README: ModelPatchLoader -> ZImageFunControlnet.
    # NOTE: exact file size wasn't confirmed in sources (it's a lightweight
    # 6-block patch, not a full diffusion model -- likely 1-3GB) so no
    # strict minimum is enforced here. Eyeball the size after first download:
    #   ls -lh models/model_patches/
    fetch_bg "Z-Image-Turbo-Fun-Controlnet-Union.safetensors" \
        "$MODELS/model_patches/Z-Image-Turbo-Fun-Controlnet-Union.safetensors" \
        "https://huggingface.co/alibaba-pai/Z-Image-Turbo-Fun-Controlnet-Union/resolve/main/Z-Image-Turbo-Fun-Controlnet-Union.safetensors" 0 \
...
if [ "$ZIMAGE" = "true" ]; then
    hdr "Custom nodes (Impact-Pack + Impact-Subpack + controlnet_aux)"
    ...
    # comfyui_controlnet_aux (Fannovel16) -- provides the DWPreprocessor
    # node used to extract a pose-ONLY skeleton from a reference image
    # for the pose-control patch to endri.json. Long-established pack,
    # predates Z-Image by years -- much more stable than the brand-new
    # Z-Image-specific pieces above.
    ...
    for node in "ComfyUI-Impact-Pack" "ComfyUI-Impact-Subpack" "comfyui_controlnet_aux"; do
