...
# └──────────────────────────────────────────────────────────┘
LORAS=(
    # "a0lorta.safetensors|https://huggingface.co/USER/REPO/resolve/main/a0lorta.safetensors"  # Mara — Krea 2
    # "L0urta.safetensors|https://huggingface.co/USER/REPO/resolve/main/L0urta.safetensors"    # L0urta — Z-Image (endri.json)
)

# ┌──────────────────────────────────────────────────────────┐
# │  WHAT TO DOWNLOAD                                        │
# └──────────────────────────────────────────────────────────┘
# KREA2  — Mara / a0lorta pipeline: krea2 turbo + qwen3vl clip + wan vae
# ZIMAGE — L0urta pipeline (endri.json, two-stage):
#          z_image_bf16 (Stage 1 base) + z_image_turbo_bf16 (Stage 2 refine)
#          + qwen_3_4b clip + ae vae + RealESRGAN_x4plus upscaler
#          Also needs face_yolov8m.pt (FaceDetailer) — covered by UPSCALERS.
KREA2="${KREA2:-true}"
ZIMAGE="${ZIMAGE:-false}"
UPSCALERS="${UPSCALERS:-true}"    # 4x-UltraSharp + face_yolov8m (needed by BOTH pipelines)
...
if [ "$ZIMAGE" = "true" ]; then
    echo "  Z-Image pipeline (endri.json — two-stage: base + turbo-refine)"
    ...
    # Stage 1 — base generation UNET (Text to Image subgraph)
    fetch_bg "z_image_bf16.safetensors" "$MODELS/unet/z_image_bf16.safetensors" \
        "https://huggingface.co/Comfy-Org/z_image_turbo/resolve/main/split_files/diffusion_models/z_image_bf16.safetensors" 7 model \
        "Comfy-Org/z_image_turbo" "split_files/diffusion_models/z_image_bf16.safetensors"

    # Stage 2 — upscale/refine UNET (Image Upscale subgraph)
    ...
    # Stage 2 upscaler — endri.json uses RealESRGAN_x4plus.safetensors specifically
    fetch_bg "RealESRGAN_x4plus.safetensors" "$MODELS/upscale_models/RealESRGAN_x4plus.safetensors" \
        "https://huggingface.co/Comfy-Org/Real-ESRGAN_repackaged/resolve/main/RealESRGAN_x4plus.safetensors" 0 model \
...
if [ "$UPSCALERS" = "true" ]; then
    ...
    # face_yolov8m — required by FaceDetailer in the Z-Image base subgraph too
